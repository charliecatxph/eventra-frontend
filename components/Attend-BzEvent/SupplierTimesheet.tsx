import {ArrowLeft, Check, ChevronDown, Clock, Minus, Pencil, User, Users, X} from "lucide-react";
import {useDispatch, useSelector} from "react-redux";
import {bizmatchSlice, setSupplierAccount, setSupplierTimeslots} from "@/features/attendBizmatchSlice";
...
export default function SupplierTimesheet() {
    ...
    const handleTimeslotInitialization = async (ts: string) => {
        try {
            ...
            setCurrentTimeslot(pv => ({
                ...pv,
                ...resData,
                noShow: registrations.filter(obj => obj.attended === "no_show").length,
                registered: registrations.filter(obj => ["registered", "present"].includes(obj.attended)).length,
                complete: registrations.filter(obj => obj.attended === "attended").length,
                fetching: {
                    requesting: false,
                    status: true,
                },
                filteredRegistrants: {
                    ...
                    registered: registrations.filter(obj => ["registered", "present"].includes(obj.attended)),
    ...
    const handleChangeAttendeeState = async (atnId, ns) => {
        if (!atnId || !["registered", "attended", "no_show", "present"].includes(ns)) return;
        try {
            const res = await axios.post(
            ...
            <div className="container w-full max-w-[1300px] mx-auto px-5">
                <div className="wrapper flex items-center justify-between">
                    <div className="logo flex gap-5 items-center">
                        <img src="/assets/petals.png" alt="MPOF2025 Petals" className="size-10"/>
                        <h1 className="text-xl font-[600] geist">MPOF2025 BizMatch Supplier Dashboard</h1>
                    </div>
                    <div className="relative">
                        <div className="flex items-center gap-3">
                            <div
                                className="logo size-7 rounded-full border-1 border-neutral-200 overflow-hidden">
        ...
        <section>
            <div className="container w-full max-w-[1300px] px-5 mx-auto">
                <div className="wrapper my-5">
                    {!timeslot && <>
                        <h1 className="font-[500] text-2xl">Hello, <span
                            className="font-[700] text-emerald-700">{bizData.supplier.name}</span>!</h1>
                        <p className="text-neutral-800">Your timesheet and appointments for this event is as
                            displayed below.</p>

                        {bizData.supplier.timeslots.length === 0 && !isFetching && fetchingStatus &&
                            <h1 className="mt-5 text-center">No timeslots issued.</h1>

                        }
                        {isFetching &&
                            <motion.div initial={{opacity: 0, y: 10}} animate={{opacity: 1, y: 0}}
                                        exit={{opacity: 0, y: -10}} key={122}
                        ...
                        <div className="grid grid-cols-3 gap-3 my-5">
                            {bizData.supplier.timeslots && !isFetching && fetchingStatus && bizData.supplier.timeslots.map((d, i) => {
                                ...
                                return <>
                                    ...
                                    <div
                                        ...
                                        <div className="p-3 bg-neutral-200 text-black">
                                            <p className="font-[600]">{moment(d.startT).format("hh:mm A")} - {moment(d.endT).format("hh:mm A")}</p>
                                        </div>
                    ...
                    {timeslot && <>
                        <div className="flex justify-between items-center">
                            <button
                                onClick={() => router.push("/attend-bizmatch", undefined, {shallow: false})}
                                className="border-1 px-5 py-1 border-neutral-100 text-sm font-[500] flex items-center gap-2 hover:bg-neutral-50 rounded-md ">
                                <ArrowLeft
                                    size="15px"/> Go Back
                            </button>
                            <div className="flex gap-2 items-center font-[600] text-xs">Your
                                supplier
                                status is:
                                <AnimatePresence mode="wait"> {bizData.supplier.open ?
                                    <motion.p initial={{scale: 0.9, opacity: 0}} animate={{scale: 1, opacity: 1}}
                                              exit={{scale: 0.9, opacity: 0}} key={1}
                                              className="bg-emerald-700 px-5 py-1 rounded-md text-white">OPEN</motion.p> :
                                    <motion.p initial={{scale: 0.9, opacity: 0}} animate={{scale: 1, opacity: 1}}
                                              exit={{scale: 0.9, opacity: 0}} key={2}
                                              className="bg-red-700 px-5 py-1 rounded-md text-white">CLOSED</motion.p>}</AnimatePresence>
                            </div>
                        </div>

                        {!currentTimeslot.fetching.requesting && currentTimeslot.fetching.status && <>
                            <div
                                className="mt-2 bg-emerald-700 text-emerald-50 px-7 py-5 rounded-md shadow-sm shadow-neutral-100 sticky top-[90px] z-[10]">

                                <div className="flex justify-between items-center">

                                    <h1 className="text-2xl font-[700] flex items-center gap-2">
                                        <Clock/> {moment(currentTimeslot.startT).format("hh:mm A")} - {moment(currentTimeslot.endT).format("hh:mm A")}
                                    </h1>
                                    <h1 className="text-sm text-emerald-50 font-[700] flex items-center gap-2">
                                        {moment(currentTimeslot.endT).format("MMMM d, yyyy")}
                                    </h1>
                                </div>
                                <div className="flex justify-between items-center">
                                    <p className="text-sm mt-1 font-[600]">{bizData.supplier.name}, {bizData.supplier.location}</p>
                                    <div className="flex items-center gap-2">
                                        <p className="text-xs mt-1 font-[500] bg-white text-black px-5 py-1 rounded-full flex items-center gap-2">
                                            <Check size="15px" strokeWidth="3"
                                                   className="text-emerald-700"/>{currentTimeslot.complete} completed
                                        </p>
                                        <p className="text-xs mt-1 font-[500] bg-white text-black px-5 py-1 rounded-full flex items-center gap-2">
                                            <Pencil size="15px" strokeWidth="3"
                                                    className="text-yellow-700"/>{currentTimeslot.registered} registered
                                        </p>
                                        <p className="text-xs mt-1 font-[500] bg-white text-black px-5 py-1 rounded-full flex items-center gap-2">
                                            <X size="15px" strokeWidth="3"
                                               className="text-red-700"/>{currentTimeslot.noShow} didn't show up at all
                                        </p>
                            ...
                            <div className="mt-5">
                                ...
                                <div className="h-full">
                                    {currentTimeslot.filteredRegistrants.complete.length === 0 &&
                                        <div
                                            className="h-[400px] atendee-table mt-5 rounded-lg overflow-hidden border-1 border-neutral-200 w-full grid place-items-center">
                                            <p>No data.</p>
                                        </div>}
                                    <AnimatePresence>
                                        {currentTimeslot.filteredRegistrants.complete.length !== 0 && (
                                            <>
                                                <div>
                                                    <motion.div initial={{opacity: 0}}
                                                        ...
                                                        <table
                                                            className="table-fixed bg-white font-[500] geist text-xs w-full">
                                                            <tr className="sticky top-0">
                                                                <th
                                                                    className="w-[200px] p-3 font-[500] bg-neutral-100 whitespace-nowrap text-left">
                                                                    Organization / E-Mail
                                                                </th>
                                                                <th
                                                                    className="w-[400px] p-3 font-[500] bg-neutral-100 whitespace-nowrap text-left">
                                                                    Represented By / Position
                                                                </th>

                                                                <th
                                                                    className="w-[400px] p-3 font-[500] bg-neutral-100  text-right">
                                                                    Options
                                                                </th>

                                                            </tr>

                                                            {currentTimeslot.filteredRegistrants.complete.map((d, i) => {
                                                                ...
                                                                return <>
                                                                    ...
                                                                    >
                                                                        <td
                                                                            className="w-[200px] p-3 max-w-[300px]">
                                                                            <div
                                                                                className="flex justify-center flex-col">
                                                                                <p className="truncate">{d.bizregistrant.orgN}</p>
                                                                        ...
                                                                        <td
                                                                            ...
                                                                            <div
                                                                                className="flex justify-center flex-col">
                                                                                <p className="truncate">{d.bizregistrant.name}</p>
                                                                        ...
                                                                        <td
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                <button
                                                                                    onClick={() => {
                                                                                        handleChangeAttendeeState(d.id, "present")
                                                                                    }}
                                                                                ...
                                                                                >
                                                                                    <Minus size="15px"/> Revert, mark as
                                                                                    present
                                                                                </button>
                            ...
                            <div className="">
                                ...
                                <div className="h-full">
                                    {currentTimeslot.filteredRegistrants.registered.length === 0 &&
                                        <div
                                            className="h-[400px] atendee-table mt-5 rounded-lg overflow-hidden border-1 border-neutral-200 w-full grid place-items-center">
                                            <p>No data.</p>
                                        </div>}
                                                    ...
                                                    <motion.div initial={{opacity: 0}}
                                                        ...
                                                        <table
                                                            ...
                                                            <tr className="sticky top-0">
                                                                ...
                                                                <th
                                                                    className="w-[80px] p-3 font-[500] bg-neutral-100 whitespace-nowrap text-left">
                                                                    Status
                                                                </th>
                                                                <th
                                                                    className="w-[400px] p-3 font-[500] bg-neutral-100  text-right">
                                                            ...
                                                            {currentTimeslot.filteredRegistrants.registered.map((d, i) => {
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <td
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                <p className="truncate uppercase">{d.attended}</p>
                                                                        ...
                                                                        <td
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                {d.attended === "present" && <>
                                                                                    <button
                                                                                        onClick={() => {
                                                                                            handleChangeAttendeeState(d.id, "registered")
                                                                                        }}
                                                                                        className="flex bg-yellow-700 hover:bg-yellow-800 text-yellow-50 font-[600] items-center gap-2 p-2   border-1 border-neutral-200 rounded-md"
                                                                                    >
                                                                                        <Minus size="15px"/> Revert,
                                                                                        mark as
                                                                                        registered
                                                                                    </button>
                                                                                    <button
                                                                                        onClick={() => {
                                                                                            handleChangeAttendeeState(d.id, "attended")
                                                                                        }}
                                                                                ...
                                                                                {d.attended !== "present" && <>
                                                                                    <button
                                                                                        onClick={() => {
                                                                                            handleChangeAttendeeState(d.id, "no_show")
                                                                                        }}
                            ...
                            <div className="">
                                ...
                                <div className="h-full">
                                    {currentTimeslot.filteredRegistrants.noShow.length === 0 &&
                                        <div
                                            className="h-[400px] atendee-table mt-5 rounded-lg overflow-hidden border-1 border-neutral-200 w-full grid place-items-center">
                                            <p>No data.</p>
                                        </div>}
                                                ...
                                                <div>
                                                    <motion.div initial={{opacity: 0}}
                                                        ...
                                                        <table
                                                            ...
                                                            {currentTimeslot.filteredRegistrants.noShow.map((d, i) => {
