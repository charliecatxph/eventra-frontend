...
export default function Events() {
  ...
  const [oEvs, setOEvs] = useState<OrdinaryEV[]>([]);
  ...
  const fetchEvents = async () => {
    try {
      ...
      const req = await axios.post(
        `${process.env.NEXT_PUBLIC_API}/fetch-events?mode=partial-ord`,
        {},
        {
          headers: {
      ...
      acs.data.forEach((ordinaryEvent: any) => {
        ...
        tmp.push({
          ...
          limitAtendee: ordinaryEvent.limitAtendee,
          location: ordinaryEvent.location,
          orgId: ordinaryEvent.organizationId,
          name: ordinaryEvent.name,
          sec_url: ordinaryEvent.coverFile,
          offset: ordinaryEvent.offsetT * -1,
          type: "Ordinary Event",
          id: ordinaryEvent.id,
        });
      });
  ...
  useEffect(() => {
    if (oEvs.length === 0) return;
    const shwlx = [...oEvs];
    setAEvs(
      shwlx.filter((ev) => !filterEvs.status || ev.status === filterEvs.status)
    );
  }, [oEvs, filterEvs]);
        ...
        <section className="eventra-container inter px-5 pb-5">
          <div className="flex gap-5 mt-5">
            ...
            <div className="main-dash w-full">
              <h1 className="text-2xl font-[600]">Events</h1>
              <div className="dash-start mt-10">
                <div className="w-full mt-5 rounded-2xl p-5  bg-white shadow-sm shadow-neutral-200 min-h-[600px]">
