...
import {
  ...
  setFetching,
  setInitialized,
} from "@/features/bizmatchEventSlice";
...
import { getBizMatchEvent } from "@/functions/getBizmatchEvent";
import io, { Socket } from "socket.io-client";
import AllSuppliers from "@/components/ViewBizMatchEvent/AllSuppliers";
...
export default function ViewBzEvent() {
  ...
  const fetchBizMatchEvent = async () => {
    try {
      ...
      const req2 = await fetchBizEventAnalytics(
        evData.data.id,
        appData.acsTok
      ).catch((e) => {
        throw new Error(e?.err);
      ...
      const status =
        moment().unix() < evData.data.startT!
          ? "Upcoming"
          : moment().unix() >= evData.data.startT! &&
            moment().unix() <= evData.data.endT!
          ? "Ongoing"
          : "Past";
  ...
  const onClientRegisterTimeslot = async (bzId) => {
    try {
      const evData = await getBizMatchEvent(
        "suppliers",
        router.query.slug as string,
      ...
      const req2 = await fetchBizEventAnalytics(bzId, appData.acsTok).catch(
        (e) => {
          throw new Error(e?.err);
        }
      );
      ...
      dispatch(
        setEvent({
          ...bzData__static.current.data,
          suppliers: [...evData.data.suppliers],
          stats: {
            ...req2.data.data.data,
          },
        })
  ...
  const onClientRegisterBz = async (bzId) => {
    try {
      const evData = await getBizMatchEvent(
        "clients",
        router.query.slug as string,
  ...
  useEffect(() => {
    ...
    socketRef.current.on("connect", () => {
      socketRef.current.emit("identify", {
        type: "EVN-ADMIN",
        bzId: bzData__static.current.data.id,
      });
    });
  ...
  return (
    <>
      <Head>
        <title>Eventra | View BizMatch Event ({router.query.slug})</title>
          ...
          >
            <div>
              <h1 className="font-[400] text-base flex gap-2 items-center">
                <ArrowLeftRight size="18px" strokeWidth={2} /> BizMatch Events{" "}
                <ChevronRight className="text-neutral-600" />{" "}
                <p className="font-[500]">
                  {bzData.fetching.mainEvent
                    ? connectionStatus
                    : bzData.data.name}
                </p>
                {atnId && (
        ...
        <section className="flex" style={{ height: "calc(100vh - 60px)" }}>
          <aside className="w-[264px] bg-neutral-50 h-full flex flex-col justify-between py-5">
            <ul className="flex flex-col gap-2 px-3 select-none">
              ...
              <li
                onClick={() => {
                  router.push(`/live-bz-display?bzId=${router.query.slug}`);
                }}
                className="py-2 text-neutral-700 hover:bg-neutral-100 px-4 rounded-lg flex gap-2 items-center cursor-pointer"
              >
                <ScreenShare size="18px" />
                Live Display
              </li>
            </ul>
              ...
              <li
                onClick={() => {
                  ...
                  modal.show({
                    ...
                    description:
                      "This feature is currently disabled. You currently using v1.7 of Eventra.",
                    onConfirm: () => {
                      modal.hide();
                    },
          ...
          <div
            className="h-full bg-neutral-100 pt-5 px-5 overflow-y-scroll"
