...
import Review from "@/components/Appointment/Review";
import { CircularProgress } from "@mui/material";
import Timesheets from "@/components/Appointment/Timesheets";
import Suppliers from "@/components/Appointment/Suppliers";
...
export default function OrdinaryEvent() {
  ...
  const validateTime = (): boolean => {
    let isValid = true;
    const updatedState: Partial<TimeData> = {};

    if (!time.date.value) {
      updatedState.date = { ...time.date, err: "Date is required" };
      isValid = false;
    } else {
      ...
      if (dateEntryUnix.getTime() < dateNow.getTime()) {
        updatedState.date = { ...time.date, err: "Date cannot be in the past" };
        isValid = false;
      }
    }

    if (!time.startT.value) {
      updatedState.startT = { ...time.startT, err: "Start time is required" };
      isValid = false;
    }

    if (!time.endT.value) {
      updatedState.endT = { ...time.endT, err: "End time is required" };
      isValid = false;
    }

    if (time.startT.value && time.endT.value && time.date.value) {
      const startUnix = getUnix(time.date.value, time.startT.value);
      const endUnix = getUnix(time.date.value, time.endT.value);
    ...
    setTime((prev) => ({ ...prev, ...updatedState }));
    return isValid;
  };
      ...
      switch (step) {
        ...
        case 5: {
          ...
          modal.show({
            ...
            description:
              "Are you sure you want to create this BizMatch event? This action cannot be undone.",
            ...
            onConfirm: async () => {
              ...
              modal.show({
                ...
                description:
                  "Please wait while we create your BizMatch event...",
                color: "success",
              });
              ...
              const payload = {
                ...
                date: Math.floor(
                  new Date(time.date.value).setHours(0, 0, 0, 0)
                ),
              ...
              try {
                const response = await axios.post(
                  `${process.env.NEXT_PUBLIC_API}/upload-biz-match`,
                  form,
                  {
                    withCredentials: true,
                ...
                modal.show({
                  ...
                  description: "BizMatch event has been created successfully.",
                ...
                modal.show({
                  ...
                  onConfirm: () => {
                    modal.hide();
                    setStep(5); // Stay on the same step
                  },
                  color: "error",
                });
              }
    ...
    <>
      <>
        <main className="min-h-screen w-full bg-[#f5f6fb] inter pb-5">
          ...
          <section>
            <div className="eventra-container-narrow pt-5">
              <button
                ...
                className="border-1 rounded-lg border-neutral-200 px-4 py-1 text-sm text-neutral-900 flex gap-2 items-center bg-white hover:bg-neutral-50 transition-colors"
              >
              ...
              <div className="steps flex gap-3 items-center mt-5 text-sm">
                <div
                  className={`step flex-1 flex items-center gap-2 ${
                    step === 1 ? "text-emerald-700" : "text-neutral-400"
                  }`}
                >
                  <div
                  ...
                  <p className="font-[500]">Information</p>
                </div>
                <div
                  className={`w-12 h-[2px] ${
                    step >= 2 ? "bg-emerald-700" : "bg-neutral-200"
                  }`}
                ></div>
                <div
                  className={`step flex-1 flex items-center gap-2 ${
                ...
                <div
                  className={`w-12 h-[2px] ${
                    step >= 3 ? "bg-emerald-700" : "bg-neutral-200"
                  }`}
                ></div>
                <div
                  className={`step flex-1 flex items-center gap-2 ${
                ...
                <div
                  className={`w-12 h-[2px] ${
                    step >= 4 ? "bg-emerald-700" : "bg-neutral-200"
                  }`}
                ></div>
                <div
                  className={`step flex-1 flex items-center gap-2 ${
                ...
                >
                  ...
                  <p className="font-[500]">Suppliers</p>
                </div>
                <div
                  className={`w-12 h-[2px] ${
                    step >= 5 ? "bg-emerald-700" : "bg-neutral-200"
                  }`}
                ></div>
                <div
                  className={`step flex-1 flex items-center gap-2 ${
                    step === 5 ? "text-emerald-700" : "text-neutral-400"
                  }`}
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center ${
          ...
          {step === 2 && <EventDates evDates={time} setEvDates={setTime} />}
          ...
          {step === 4 && (
            <Suppliers data={suppliers} onDataChange={setSuppliers} />
          )}
          ...
          <div className="flex justify-end pt-5 eventra-container-narrow">
            ...
            {!isUploading && (
              <button
                ...
                className="flex gap-2 items-center text-sm bg-emerald-700 hover:bg-emerald-800 text-white px-5 py-1 rounded-md font-[500] transition-colors"
              >
                {step === 1 && "Proceed to Customization"}
                {step === 2 && "Proceed to Timesheets"}
                {step === 3 && "Proceed to Suppliers"}
                {step === 4 && "Proceed to Summary"}
                {step === 5 && "Upload BizMatch Event"}
