...
import OrdinaryEvent from "@/screens/OrdinaryEvent";
import {AnimatePresence, motion} from "framer-motion";
import {ArrowLeft, Check, ChevronDown, ChevronUp, DoorOpen,} from "lucide-react";
...
export default function CreateEvent() {
    ...
    const [renderEvType, setRenderEvType] = useState<boolean>(false); // true ... /false
    ...
    return (
        <>
            <Head>
                <title>Eventra | Create Event</title>
                <link rel="icon" href="/favicon.ico"/>
            </Head>
            {!evType && (
                <>
                    <main className="min-h-screen w-full bg-[#f5f6fb] inter">
                        <header className="bg-[#f5f6fb]  sticky top-0 left-0 z-[999]  py-7">
                            <div className="eventra-container flex  justify-between items-center select-none px-5">
                                ...
                                <div
                                    className="relative usr-log shadow-sm shadow-neutral-200 bg-white rounded-full px-4 py-1.5 items-center flex gap-2">
                                    <div
                                        className="avatar h-[25px] w-[25px] rounded-full overflow-hidden border-2 border-emerald-600">
                        ...
                        <section>
                            <div className="eventra-container-narrow pt-5">
                                ...
                                >
                                    <ArrowLeft size="15px"/> Return to Dashboard
                                </button>
                                <h1 className="font-[600] text-2xl text-center mt-10">
                                    Event Creation
                                </h1>
                                <p className="text-sm text-center text-neutral-900">
                                    Choose your event type below to proceed.
                                </p>
                                <div className="ev-types flex gap-2 mt-5">
                                    <motion.div
                                        initial={{opacity: 0, y: 10}}
                                        animate={{opacity: 1, y: 0, transition: {delay: 0.25}}}
                                        exit={{opacity: 0, y: 10}}
                                        className="event-type bg-white px-5 py-8 w-1/2 rounded-lg flex flex-col justify-between gap-10"
                                    >
                                        <div>
                                            <h1 className="font-[600] text-lg">Ordinary Event</h1>
                                            <p className="text-sm text-neutral-900">
                                                Set an event name, start and end times, location,
                                                description, cover, walk-in settings, and atendee limit.
                                            </p>
                                            <ul className="text-sm mt-5 flex flex-col gap-1">
                                                ...
                                                <li className="flex gap-2 items-center">
                                                    <Check
                                                        ...
                                                        strokeWidth={3}
                                                    />
                                                    On-Demand Event Data Exportation (.pdf, .csv)
                                                </li>
                                            </ul>
                                        </div>
                                        <div>
                                            <button
                                                onClick={() => setEvType("ord")}
                                                className="text-sm bg-black hover:bg-neutral-900 text-white px-5 py-1 rounded-md"
                                            >
                                                Select
                                            </button>
                                        </div>
                                    ...
                                    >
                                        <div>
                                            <h1 className="font-[600] text-lg">BizMatch Event</h1>
                                            <p className="text-sm text-neutral-900">
                                                Holds the same characteristics as an ordinary event, but
                                                you instead issue timeslots for atendees to register on.
                                            </p>
                                            <ul className="text-sm mt-5 flex flex-col gap-1">
                                                <li className="flex gap-2 items-center">
                                                    <Check
                                                ...
                                                <li className="flex gap-2 items-center">
                                                    <Check
                                                        ...
                                                        strokeWidth={3}
                                                    />
                                                    Supplier Access Key and Schedule View
                                                </li>
                                            </ul>
                                        </div>
                                        <div>
                                            <button
                                                onClick={() => setEvType("apt")}
                                                className="text-sm bg-black hover:bg-neutral-900 text-white px-5 py-1 rounded-md"
                                            >
                                                Select
                                            </button>
